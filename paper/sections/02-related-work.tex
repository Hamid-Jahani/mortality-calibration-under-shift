...
and one age range at a time. Three of its findings frame ours. Of the two
ages it plots, it reports that ``the age 65 forecasts have a notable
...
bias''---better behaviour at the higher age, and so the ex-ante
alternative to \Hfour. It notes, in a footnote, that ``the
...
registered ``under-cover in the shift regime for every model family''
clause was therefore falsified
...
Conformal prediction reached mortality forecasting only recently, in a
2025--2026 line of work that predates this study:
\citet{shang2025intervals} use split conformal intervals as one calibration
device when forecasting the age distribution of deaths,
\citet{shang2026conformal} introduces split and sequential conformal
prediction for age-specific mortality rates, and
\citet{shang2026subnational} extends both to subnational mortality. All
three introduce the mechanism under a single functional forecasting family
and evaluate its coverage in ordinary years; none audits conformal coverage
across a designated structural break, places conformal beside other
uncertainty mechanisms, or measures joint path coverage. Beyond that line
of work, a web-scale search recorded in the prior-art map found no
application of conformal prediction to mortality forecasting; in the
52-paper corpus itself, ``conformal'' has no hit in any mortality text, and
neither do ``Monte Carlo dropout'' or ``deep
...
conformal arms inside that design and across the break, the
marginal-to-joint gap, the derived-quantity coverage
