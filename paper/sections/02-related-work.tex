...
their multi-period-ahead density forecasts, applied ex post to English and
Welsh males. \todo{VERIFY against Dowd et al.\ (2010b) once read: the exact
backtests used (probability-integral or hit-rate based), the horizons, and
the finding that parameter-uncertainty-inclusive densities are ``more
plausible''; adjust the novelty claim in Section~\ref{sec:intro}
accordingly} Later members of the programme include the Bayesian
two-population model of \citet{cairns2011bayesian}, the general procedure
for model construction of \citet{hunt2014general} and the CBDX workhorse of
\citet{dowd2020cbdx}. \citet{booth2008mortality} review the wider field.
The \pkg{StMoMo} package \citep{villegas2018stmomo} implements the family in
a common generalised-linear framework and is the numerical oracle for the
classical arms of this study.
...
\citet{marino2021deepening} deepen Lee--Carter and add interval estimation
for the future $\kappa_t$; \citet{miyata2022extending} replace the two-stage
procedure by a variational autoencoder fit in one stage and obtain intervals
without MCMC; \citet{schnurch2022point} is the one neural mortality paper
whose title promises interval forecasts. \todo{Schn\"urch and Korn (2022) not
yet read; describe their interval construction and any coverage evidence
here} Adjacent non-neural machine-learning comparators are the
gradient-boosted multi-population model of \citet{miao2025gradient}, the
tree-based analysis of \citet{deprez2017machine}, the pension-fund
application of \citet{demelo2025actuarial}, and the Markov-switching
Bayesian VAR on an age-partitioned Lee--Carter of \citet{fu2025applying},
which is a Bayesian rather than a neural route to uncertainty under
structural change.
...
they exist; coverage computed once, without a sharpness penalty; proper
scores never. The four neural families in our grid
(Section~\ref{sec:design-families}) are drawn from it.
...
density and CRPS; \citet{liu2020bayesian} fit a Bayesian Poisson log-normal
model with regularised time structure. These are the models whose predictive
distributions are honest by construction, and their evaluation practice is
the one we adopt for every family; a multi-output GP is one of the ten
families audited.
...
stochastic multi-population model; \citet{goes2023bayesian} extend
...
risk to a multi-population model; \citet{vanberkum2022estimating} extend the
...
perturbation because real post-shock data did not yet exist. None of these
papers evaluates prediction intervals that were fit before 2020 against what
happened in 2020--2024. That is the gap this audit fills, and the papers
above are what a modeller would reach for afterwards to repair a failure we
document; repairing it is not our contribution.
...
recalibration. \citet{dawid2014theory} survey the theory; \citet{yanagisawa2023proper}
extend proper scoring to survival-time distributions, which is the natural
target when the object of interest is a lifetime rather than a rate.
...
forecasts, where the object of interest is the whole path,
\citet{sun2024copula} calibrate a joint band through a copula of the
per-horizon scores; related constructions are the copula-based multi-target
sets of \citet{messoudi2021copula}, the multi-horizon bands of
\citet{stankeviciute2021conformal} and the functional bands of
\citet{diquigiovanni2022conformal}. \citet{sun2025conformal} address change
points directly. \citet{angelopoulos2023gentle} is the standard primer.

We found no application of conformal prediction to mortality forecasting.
Three conformal mechanisms enter our grid as \emph{audited arms}, on the
same footing as the native and ensemble mechanisms; we make no claim that
they fix anything, and we deliberately do not invoke the finite-sample
...
common shock observed in twenty correlated populations, ordinary
Diebold--Mariano standard errors overstate the information available; we
cluster by population and use the wild cluster bootstrap of
\citet{cameron2008bootstrap} with the six-point weights of
