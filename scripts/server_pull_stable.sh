...
#   <NODE_B_IP> (two hops)  — 14 populations, parts named POP__MODEL.parquet
#   <BASTION> / .49 (one hop)   — SWE,DNK,ISL,BEL,NOR,CHE with --origins halves,
...
PY=<HOME>/venvs/mortcal-cpu/Scripts/python.exe
...
echo "== <NODE_B_IP> (two-hop): untagged parts =="
"$SSH" -o BatchMode=yes <BASTION> \
  "ssh <NODE_B_IP> \"cd $REPO && tar czf - --ignore-failed-read results/stable.parts results/stable_gp.parts results/logs/stable* 2>/dev/null\"" \
  | tar xzf - -C "$DEST" --strip-components=1

echo "== <BASTION> / .49 (single-hop): origin-tagged parts =="
"$SSH" -o BatchMode=yes <BASTION> \
