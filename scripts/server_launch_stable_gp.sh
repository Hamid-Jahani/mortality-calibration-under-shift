#!/usr/bin/env bash
# Launch the STABLE GP pass on the bastion (.49 / <NODE_A>). Runs ON the
# server; ship + run from the laptop (Gaming profile, single hop):
#
#   cat src/mortcal/runner.py | /c/Windows/System32/OpenSSH/ssh.exe <BASTION> \
#     'cat > ~/mortality-calibration-under-shift/src/mortcal/runner.py'
#   cat scripts/server_launch_stable_gp.sh | /c/Windows/System32/OpenSSH/ssh.exe <BASTION> \
#     'cat > ~/mortality-calibration-under-shift/scripts/server_launch_stable_gp.sh'
#   /c/Windows/System32/OpenSSH/ssh.exe <BASTION> \
