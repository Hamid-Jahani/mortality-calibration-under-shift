...
`.venv` was built under Windows profile `<HOME>`, which no longer
exists (only `C:\Users\ASUS`). `uv` is not on PATH for this profile either, so
the project notes' `~/.local/bin/uv.exe` is stale. Patching `.venv/pyvenv.cfg` is NOT
...
- Do NOT resume workflow `the dead sweep run` (dead; resume clobbers manual RH
