...
**User decision: keep the selected title and build the family axis**
...
no visible NVIDIA driver despite the project notes' GPU claim; the cu124 wheel
...
then the GP passes. Check: `ssh <BASTION> 'ssh <NODE_B_IP> "tail -3
...
- **Compute authorization**: the user owns both machines; the bastion
  `<NODE_A>` (24 cores, 23 GB with ~8 GB free, load ~4 from its own
...
- **10:20 node time — STABLE regime started on the bastion** (`<NODE_A>`,
...
The owed literature search (Exa, web search, arXiv, Crossref; queries dated in
...
|---|---|---|---|
| `.47` <NODE_B> | staging, 21 GB free | 4 | BEL CHE DNK FIN ISL JPN LUX NOR PRT SWE USA (11) |
| `.49` <NODE_A> | **production**, 11 GB free | 2 | TWN EST LTU LVA SVK (5) |
